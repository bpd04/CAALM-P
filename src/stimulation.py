...
def generate_antiphase_wave(freq_hz, phase_rad, amplitude, block_n, fs=FS, phase_offset=0.0):
    ...
    but shifted by pi (180 degrees) + optional phase correction.
    """
...
def map_control_to_amplitude(envelope_mean, gain_multiplier=1.0, amplitude_scale=55e-6):
    ...
    amplitude_scale controls the maximum simulation waveform amplitude.
    """
...
def _estimate_stim_frequency_hz(stim_wave, fs=FS):
    ...
    This is only for instrumentation, not for control.
    """
...
def apply_oscillation_suppression_to_block(
    ...
    """
    Instrumented old-law plant model.

    IMPORTANT:
    This preserves the original strong Step 22B-style behavior:
    - isolate oscillatory component of next block
    - preserve residual / non-oscillatory component
    - suppress only the oscillatory component
    - suppression depends on:
        * patient_response
        * normalized current
        * anti-phase alignment between osc and stim_wave

    controlled_block = residual + (1 - beta) * osc

    where beta is still:
        beta = suppression_gain * patient_response * current_norm * antiphase_match
    and then clipped to [0, max_suppression]

    The upgrade here is instrumentation only.
    """
    ...
    # if osc and stim are strongly negatively correlated -> good suppression
    corr = _safe_corr(osc_component, stim_wave)
    antiphase_match = max(0.0, -corr)  # 0 to 1 ideally
    ...
    meta = {
        ...
        # compatibility keys for existing experiment_runner/main instrumentation
